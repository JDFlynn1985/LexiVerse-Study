...
import { User, Cpu, Loader2, ArrowRight, ExternalLink, Key, Globe, Brain, Sparkles, Server } from 'lucide-react';
...
export const ProfileView = memo(({ 
  ...
  const KEY_FIELDS = [
    { id: 'googleKey', label: 'Google AI API Key', provider: 'google', icon: Globe },
    { id: 'openaiKey', label: 'OpenAI API Key', provider: 'openai', icon: Cpu },
    { id: 'anthropicKey', label: 'Anthropic API Key', provider: 'anthropic', icon: Brain },
    { id: 'mistralKey', label: 'Mistral API Key', provider: 'mistral', icon: Sparkles },
    { id: 'deepseekKey', label: 'DeepSeek API Key', provider: 'deepseek', icon: Cpu },
    { id: 'xaiKey', label: 'xAI (Zai) API Key', provider: 'xai', icon: Cpu }
  ];
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="shadow-lg border-primary/10">
            <CardHeader>
              ...
              <CardDescription>Manage your API keys for various cloud providers.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                {KEY_FIELDS.map((field) => (
                  <div key={field.id} className="space-y-2 p-4 bg-muted/30 rounded-lg border">
                    ...
                    <Input 
                      type="password" 
                      placeholder={`Paste your ${field.provider} key...`} 
                      value={aiPrefs[field.id] || ''} 
                      onChange={e => saveAiPreferences({ [field.id]: e.target.value })} 
                    />
                  </div>
                ))}
              </div>
