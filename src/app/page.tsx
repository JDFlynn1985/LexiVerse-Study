...
const DashboardView = memo(({ t, effectiveApiKey, isLocalMode, aiPrefs, assistantTerm, setAssistantTerm, handleSearch, isLoading, historyItems, setActiveTab }: any) => (
...
const ChatView = memo(({ chatMode, setChatMode, userProfile, userInstitutionName, messages, user, newMessage, setNewMessage, handleSendMessage, chatEndRef }: any) => (
...
const SynthesisView = memo(({ synthesisText, setSynthesisText, handleSynthesisAction, isLoading, synthesisResult, integrityResult, bibResult }: any) => (
...
const TheologyView = memo(({ theologyTerm, setTheologyTerm, handleSearch, isLoading, theologyResult }: any) => (
  <div className="space-y-8 animate-in fade-in duration-500">
    <header>
      <h1 className="text-3xl font-bold font-headline flex items-center gap-3">
...
const LexiconView = memo(({ handleSearch, isLoading, lexiconResult }: any) => (
  <div className="space-y-8 animate-in fade-in duration-500">
    <header>
      <h1 className="text-3xl font-bold font-headline flex items-center gap-3">
    ...
    <div className="flex gap-4">
      <Input placeholder="Enter Strong's Number (e.g. G3056)..." className="h-12 shadow-sm" onKeyDown={e => e.key === 'Enter' && handleSearch(e.currentTarget.value, 'lexicon')} />
      <Button size="lg" onClick={() => { const el = document.querySelector('input[placeholder*="Strong"]') as HTMLInputElement; if (el) handleSearch(el.value, 'lexicon'); }} disabled={isLoading}>
        {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5" />}
      </Button>
    </div>
...
const AssistantView = memo(({ assistantTerm, setAssistantTerm, handleSearch, isLoading, assistantResult }: any) => (
  <div className="space-y-8 animate-in fade-in duration-500">
    <header>
      <h1 className="text-3xl font-bold font-headline flex items-center gap-3">
